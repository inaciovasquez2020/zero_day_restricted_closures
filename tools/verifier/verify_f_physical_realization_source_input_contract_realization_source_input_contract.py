...
def req(cond, msg):
    if not cond:
        raise SystemExit("F_PHYSICAL_REALIZATION_SOURCE_INPUT_CONTRACT_REALIZATION_SOURCE_INPUT_CONTRACT_FAIL: " + msg)

def main():
    contract = load("core/f_physical_realization_source_input_contract_realization_source_input_contract_surface.json")
    source = load("core/f_physical_realization_source_input_contract_realization_source_target_surface.json")
    realization = load("core/f_physical_realization_source_input_contract_realization_target_surface.json")
    source_contract = load("core/f_physical_realization_source_input_contract_surface.json")
    ...
    req(contract.get("object") == "F_physical_realization_source_input_contract_realization_source_input_contract_surface", "contract object changed")
    req(contract.get("status") == "input_contract_surface_only", "contract status changed")
    req(contract.get("target_source") == "F_physical_realization_source_input_contract_realization_source", "target source changed")
    req(contract.get("would_supply") == "F_physical_realization_source_input_contract_realization_source", "would-supply changed")
    ...
    req(source.get("available") is False and source.get("inhabited") is False, "realization source-input source became available")
    req(realization.get("available") is False and realization.get("inhabited") is False, "source-input realization became available")
    req(source_contract.get("available") is False and source_contract.get("inhabited") is False, "source-input contract became available")
    ...
    print("F_PHYSICAL_REALIZATION_SOURCE_INPUT_CONTRACT_REALIZATION_SOURCE_INPUT_CONTRACT_OK")
