...
def req(cond, msg):
    if not cond:
        raise SystemExit("F_PHYSICAL_REALIZATION_SOURCE_INPUT_CONTRACT_REALIZATION_SOURCE_TARGET_FAIL: " + msg)

def main():
    target = load("core/f_physical_realization_source_input_contract_realization_source_target_surface.json")
    ...
    req(target.get("object") == "F_physical_realization_source_input_contract_realization_source_target_surface", "target object changed")
    req(target.get("status") == "source_target_surface_uninhabited", "target status changed")
    req(target.get("target") == "F_physical_realization_source_input_contract_realization_source", "target name changed")
    req(target.get("would_supply") == "F_physical_realization_source_input_contract_realization", "would-supply changed")
    req(target.get("would_realize") == "F_physical_realization_source_input_contract", "would-realize changed")
    ...
    req(contract.get("available") is False and contract.get("inhabited") is False, "source-input contract became available")
    req(source.get("available") is False and source.get("inhabited") is False, "derivation realization source became available")
    ...
    print("F_PHYSICAL_REALIZATION_SOURCE_INPUT_CONTRACT_REALIZATION_SOURCE_TARGET_OK")
