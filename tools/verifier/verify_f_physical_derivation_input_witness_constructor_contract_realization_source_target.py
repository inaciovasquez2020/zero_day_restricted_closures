...
def req(cond, msg):
    if not cond:
        raise SystemExit("F_PHYSICAL_DERIVATION_INPUT_WITNESS_CONTRACT_REALIZATION_SOURCE_TARGET_FAIL: " + msg)

def main():
    target = load("core/f_physical_derivation_input_witness_constructor_contract_realization_source_target_surface.json")
    ...
    req(target.get("object") == "F_physical_derivation_input_witness_constructor_contract_realization_source_target_surface", "target object changed")
    req(target.get("status") == "source_target_surface_uninhabited", "target status changed")
    req(target.get("target") == "F_physical_derivation_input_witness_constructor_contract_realization_source", "target name changed")
    req(target.get("would_supply") == "F_physical_derivation_input_witness_constructor_contract_realization", "would-supply target changed")
    ...
    print("F_PHYSICAL_DERIVATION_INPUT_WITNESS_CONTRACT_REALIZATION_SOURCE_TARGET_OK")
