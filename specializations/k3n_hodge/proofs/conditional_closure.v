...
K3nMonodromyStableRequiredInventory(X) :=
  finite_index :
    Finite(ZeroDayRequiredK3nHodgeClasses(X).I)

  index_action :
    forall sigma : Mon(X),
      ZeroDayRequiredK3nHodgeClasses(X).I
        ->
      ZeroDayRequiredK3nHodgeClasses(X).I

  index_action_identity :
    forall i,
      index_action(identity,i) = i

  index_action_composition :
    forall sigma tau i,
      index_action(sigma * tau,i)
        =
      index_action(
        sigma,
        index_action(tau,i)
  ...
  degree_preserved :
    forall sigma i,
      degree(index_action(sigma,i))
        =
      degree(i)

  class_equivariant :
    forall sigma i,
      ZeroDayRequiredK3nHodgeClasses(X).class(
        index_action(sigma,i)
      )
        =
      sigma_acts_on_cohomology(
        ZeroDayRequiredK3nHodgeClasses(X).class(i)
      )


Theorem finite_monodromy_stable_required_inventory_gives_finite_class_orbits:
  For X with K3nMonodromyStableRequiredInventory(X),
...
Proof:
  ...
  By class_equivariant, every monodromy translate of class(i)
  is equal to class(j) for some

    j : ZeroDayRequiredK3nHodgeClasses(X).I.
  ...
  Hence
    ...
    Orbit_Mon(X)(class(i))
      subset
    {
      ZeroDayRequiredK3nHodgeClasses(X).class(j)
      |
      j in ZeroDayRequiredK3nHodgeClasses(X).I
    }.

  By finite_index, the required-class index type I is finite.

  Therefore the set on the right is finite.

  Hence the monodromy orbit of class(i) is finite.
Qed.


Theorem finite_required_class_orbit_gives_finite_degree_four_quotient_orbit:
  For X and c in H^4(X,Q),
...
Proof:
  The quotient projection
    ...
    pi4_X :
      H^4(X,Q) -> K3nDegreeFourQuotient(X)

  is monodromy-equivariant because SH^4(X,Q) is generated by H^2
  and is preserved by the monodromy cohomology-ring action.

  Therefore the quotient orbit of pi4_X(c) is the image of
  the finite orbit of c.

  An image of a finite set is finite.
Qed.


Corollary monodromy_stable_required_inventory_gives_degree_four_finite_orbit:
  For n >= 4 and X of K3^[n]-type,
...
Proof:
  Let R : K3nMonodromyStableRequiredInventory(X).
  Let i with degree(i) = 2.

  Apply
    finite_required_class_orbit_gives_finite_degree_four_quotient_orbit.

  Apply
    finite_monodromy_stable_required_inventory_gives_finite_class_orbits
  using R and i.
Qed.


K3nMonodromyStableComputableClosureReduction(X,n) :=
  monodromy_stable_required_inventory :
    K3nMonodromyStableRequiredInventory(X)
...
Theorem k3_n_type_monodromy_stable_computable_reduction_implies_zero_day_closure:
  For n >= 4 and X of K3^[n]-type,
...
Proof:
  Let R : K3nMonodromyStableComputableClosureReduction(X,n).

  Apply
    k3_n_type_computable_degree_four_reduction_implies_zero_day_closure.

  Supply non_degree_four_required_classes_in_SH from R.

  Supply degree_four_required_quotients_have_finite_orbit using
    monodromy_stable_required_inventory_gives_degree_four_finite_orbit
  and R.monodromy_stable_required_inventory.

  Supply degree_four_required_scalar_obstruction_vanishes from R.
Qed.

Boundary:
  the independent finite-orbit hypothesis for degree-four required classes is removed
  it is replaced by explicitly assumed finite inventory plus monodromy stability and equivariance
  finiteness and monodromy stability of the required-class inventory remain unproved
  scalar obstruction vanishing remains unproved
  non-degree-four required classes are not proved to lie in SH
  no required-class inventory element is manufactured
  the stopped K3^[3] inventory branch remains stopped
  unconditional ZeroDayClosure for n >= 4 is not claimed
